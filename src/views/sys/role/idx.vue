...
<template>
    <div class="layout">
        <Row>
            ...
            <span  style="margin: 0 10px;"><Button type="primary" icon="search" @click="init">搜索</Button></span>
        </Row>
        <br>
        <!--表单-->
            ...
            edit(params){
                ...
                Util.ajax.post("/api/sys/role/info",{id:params.row.id}).then(function (response) {
                    if(response.data.code=="100"){
                        self.clearFormItem();
                        self.$refs.resourceTree.checkResource(response.data.sysRole.resourcesList);
                        self.formItem = response.data.sysRole;
